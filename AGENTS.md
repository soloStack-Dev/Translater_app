...
## Project layout
- `app/` — App Router pages: `page.tsx` (home), `features/page.tsx`, `voice/page.tsx`, `api/tts/route.ts` (Sarvam proxy). Layout uses `LayoutProps<"/">` typing (generated by `next dev`).
...
- API key is in `.env` as `SARVAM_API` (gitignored) — never hardcode it. Reads as `process.env.SARVAM_API` in `app/api/tts/route.ts`.
- Flow: user selects language (`hi-IN`, `ta-IN`, `ml-IN`, `kn-IN`) → types English text → `POST /api/tts` → route runs `client.text.translate(...)` then `client.textToSpeech.convert(...)` and returns base64 mp3 + translated text.
